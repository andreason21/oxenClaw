...
from oxenclaw.security.net.policy import NetPolicy, merge_policies
...
class BrowserPolicy:
    ...
    def from_env(cls, env: dict[str, str] | None = None) -> BrowserPolicy:
        """Build from `OXENCLAW_BROWSER_*` env vars (used by gateway boot)."""
        src = env if env is not None else os.environ
        from oxenclaw.security.net.policy import policy_from_env

        net = policy_from_env(src)
        kwargs: dict = {"net": net}
        if "OXENCLAW_BROWSER_MAX_PAGES" in src:
            kwargs["max_concurrent_pages"] = int(src["OXENCLAW_BROWSER_MAX_PAGES"])
        if "OXENCLAW_BROWSER_TIMEOUT_MS" in src:
            kwargs["default_timeout_ms"] = int(src["OXENCLAW_BROWSER_TIMEOUT_MS"])
        if src.get("OXENCLAW_BROWSER_ALLOW_DOWNLOADS", "").lower() in ("1", "true", "yes"):
            kwargs["allow_downloads"] = True
        if src.get("OXENCLAW_BROWSER_ALLOW_WS", "").lower() in ("1", "true", "yes"):
            kwargs["allow_websockets"] = True
        if "OXENCLAW_BROWSER_PROFILE_DIR" in src:
            kwargs["persistent_profile_dir"] = Path(src["OXENCLAW_BROWSER_PROFILE_DIR"])
        if "OXENCLAW_BROWSER_USER_AGENT" in src:
            kwargs["user_agent"] = src["OXENCLAW_BROWSER_USER_AGENT"]
        return cls(**kwargs)
