...
async def test_present_stores_and_fans_out() -> None:
    ...
    resp = await _call(router, "canvas.present", {
        "agent_id": "a", "html": "<!doctype html><p>x</p>", "title": "T",
    })
...
async def test_navigate_refuses_http_url() -> None:
    router, _, _ = _setup()
    resp = await _call(router, "canvas.navigate",
                       {"agent_id": "a", "url": "https://evil.test/"})
    assert resp.result["ok"] is False
...
async def test_navigate_allows_data_url() -> None:
    router, _, _ = _setup()
    resp = await _call(router, "canvas.navigate",
                       {"agent_id": "a", "url": "data:text/html,<p>x</p>"})
    assert resp.result["ok"] is True
...
async def test_get_state_round_trip() -> None:
    router, _, _ = _setup()
    await _call(router, "canvas.present",
                {"agent_id": "a", "html": "<p>x</p>", "title": "T"})
...
async def test_eval_round_trip_with_simulated_dashboard() -> None:
    ...
    async def fake_dashboard() -> None:
        ...
        await _call(router, "canvas.eval_result", {
            "request_id": evt.request_id, "ok": True, "value": 42,
        })

    dashboard = asyncio.create_task(fake_dashboard())
    resp = await _call(router, "canvas.eval", {
        "agent_id": "a", "expression": "2+2", "timeout_seconds": 2.0,
    })
    await dashboard
...
async def test_eval_times_out_when_no_dashboard() -> None:
    ...
    await _call(router, "canvas.present", {"agent_id": "a", "html": "<p>x</p>"})
    resp = await _call(router, "canvas.eval", {
        "agent_id": "a", "expression": "1", "timeout_seconds": 0.2,
    })
    assert resp.error is None
    assert resp.result["ok"] is False
