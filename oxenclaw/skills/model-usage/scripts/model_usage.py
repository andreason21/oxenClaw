...
def run_codexbar_cost(provider: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError("codexbar not found on PATH. Install CodexBar CLI first.") from exc
    except subprocess.CalledProcessError as exc:
        raise RuntimeError(f"codexbar cost failed (exit {exc.returncode}).") from exc
    try:
        payload = json.loads(output)
    except json.JSONDecodeError as exc:
        raise RuntimeError(f"Failed to parse codexbar JSON output: {exc}") from exc
