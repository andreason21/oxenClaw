...
description: |
  ...
  click links, and fill forms. Every request runs through oxenClaw's
...
requires:
  ...
  env_marker: OXENCLAW_ENABLE_BROWSER
notes:
  - "Disabled by default. Set OXENCLAW_ENABLE_BROWSER=1 (or config.yaml browser.enabled: true) and provide an allowlist via OXENCLAW_NET_ALLOW_HOSTS."
  - "Run `playwright install chromium` once after `pip install 'oxenclaw[browser]'`."
...
to any URL whose host is not in `OXENCLAW_NET_ALLOW_HOSTS` (or the
