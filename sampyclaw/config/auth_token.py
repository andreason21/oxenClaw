...
`--auth-token` flag, no `SAMPYCLAW_GATEWAY_TOKEN` env var), generate a
random one and persist it under `~/.sampyclaw/gateway-token`. Subsequent
...
2. `SAMPYCLAW_GATEWAY_TOKEN` environment variable
3. The persisted token file (`~/.sampyclaw/gateway-token`)
...
`SAMPYCLAW_GATEWAY_TOKEN` and the file path is never read or written.
"""
...
def token_file_path(paths: SampyclawPaths | None = None) -> Path:
...
def resolve_or_generate_token(
    *,
    ...
    env_token = env_map.get("SAMPYCLAW_GATEWAY_TOKEN")
