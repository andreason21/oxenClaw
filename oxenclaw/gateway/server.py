...
if TYPE_CHECKING:
    from oxenclaw.observability import ReadinessChecker
...
def _resolve_auth_token(explicit: str | None) -> str | None:
    ...
    Precedence: explicit constructor argument → `OXENCLAW_GATEWAY_TOKEN`
...
def _resolve_allowed_origins(
    ...
    Precedence: explicit kwarg → `OXENCLAW_ALLOWED_ORIGINS` env var
    ...
    if raw is None:
        env = os.environ.get("OXENCLAW_ALLOWED_ORIGINS")
        if env:
...
def _set_token_cookie(response: Response, token: str | None) -> None:
    """Set the `oxenclaw_token` cookie on `response`. No-op if `token` is
...
TOKEN_COOKIE_NAME = "oxenclaw_token"
...
class GatewayServer:
    def __init__(
        ...
        *,
        ...
        #   2. `OXENCLAW_ALLOWED_ORIGINS` env var (comma-separated)
        ...
        if self._auth_token is None:
            logger.warning(
                "gateway started WITHOUT auth — set OXENCLAW_GATEWAY_TOKEN "
