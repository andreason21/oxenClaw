...
Mirrors the openclaw `SsrFPolicy` shape with the additions oxenClaw
...
def policy_from_env(env: dict[str, str] | None = None) -> NetPolicy:
    """Build a baseline policy from `OXENCLAW_NET_*` env vars.
    ...
    return NetPolicy(
        allowed_hostnames=_csv(src.get("OXENCLAW_NET_ALLOW_HOSTS")),
        denied_hostnames=_csv(src.get("OXENCLAW_NET_DENY_HOSTS")),
        allow_private_network=src.get("OXENCLAW_NET_ALLOW_PRIVATE", "").lower()
        in ("1", "true", "yes"),
        allow_loopback=src.get("OXENCLAW_NET_ALLOW_LOOPBACK", "").lower() in ("1", "true", "yes"),
        extra_allowed_ports=_ports(src.get("OXENCLAW_NET_EXTRA_PORTS")),
    )
