...
The sampyClaw security model assumes "the agent runs as the local OS
...
- Non-loopback binds raise `RemoteBindRefused` unless the operator
  ...
  `SAMPYCLAW_ALLOW_NON_LOOPBACK=1` in the environment.
...
ENV_OPT_IN = "SAMPYCLAW_ALLOW_NON_LOOPBACK"
