"""Create / restore / verify oxenClaw backups."""
...
DEFAULT_BACKUP_PREFIX = "oxenclaw-backup"
...
def create_backup(
    ...
    *,
    ...
    """Create a `.tar.gz` snapshot of the oxenClaw home dir.
    ...
    home = resolved.home
    if not home.exists():
        raise FileNotFoundError(f"oxenClaw home does not exist: {home}")
    ...
    # archive path.
    with tempfile.TemporaryDirectory(prefix="oxenclaw-backup-") as tmpdir_str:
        ...
        with tarfile.open(archive_path, "w:gz") as tar:
            tar.add(staging, arcname="oxenclaw-backup")
...
def _read_manifest_from_archive(archive: Path) -> BackupManifest:
    with tarfile.open(archive, "r:gz") as tar:
        try:
            member = tar.getmember("oxenclaw-backup/MANIFEST.json")
        except KeyError as exc:
            raise ValueError(
                f"archive {archive} has no MANIFEST.json — not a oxenClaw backup"
            ) from exc
...
def verify_backup(archive: Path) -> BackupManifest:
    ...
    with tarfile.open(archive, "r:gz") as tar:
        for rel, expected in manifest.files.items():
            try:
                member = tar.getmember(f"oxenclaw-backup/{rel}")
            except KeyError as exc:
...
def restore_backup(
    ...
    *,
    ...
    """Extract `archive` into the oxenClaw home dir.
    ...
    with tarfile.open(archive, "r:gz") as tar:
        for rel in manifest.files:
            member_name = f"oxenclaw-backup/{rel}"
            try:
