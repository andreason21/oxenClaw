...
— wrappers in `sampyclaw.pi.providers.*` do that translation.

`AgentTool` is intentionally compatible with `sampyclaw.agents.tools.Tool`
...
class AgentTool(Protocol):
    ...
    Compatible with the existing `sampyclaw.agents.tools.Tool` Protocol —
