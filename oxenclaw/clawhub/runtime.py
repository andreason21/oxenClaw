...
- Creates an ephemeral workspace dir under `~/.oxenclaw/skill-workspaces/`.
...
def _workspace_root(paths: OxenclawPaths) -> Path:
...
def prepare_skill_runtime(
    ...
    *,
    ...
    if config.kind == "ephemeral":
        ws = Path(tempfile.mkdtemp(prefix=f"oxenclaw-{skill.slug}-", dir=_workspace_root(paths)))
    else:
