...
from sampyclaw.memory.hybrid import bm25_rank_to_score
from sampyclaw.memory.models import FileEntry, MemoryChunk
    ...
    def ensure_schema_meta(self, provider: str, model: str, dims: int) -> None:
        ...
        if "embedding_provider" in existing and existing["embedding_provider"] != provider:
            raise ValueError(
                ...
                f"Run `sampyclaw memory rebuild --yes` to reset the index."
            )
        now = time.time()
        rows = [
