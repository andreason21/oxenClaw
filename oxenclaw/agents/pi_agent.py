...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "decisions live in a vector-indexed memory store. Two tools:\n"
    "  - `memory_save(text, tags?)` — persist a stable fact whenever\n"
    "    the user asks you to remember something OR you learn a durable\n"
    "    preference (their name, role, deadline, tooling preference).\n"
    "    Skip ephemeral details and chat transcripts.\n"
    "  - `memory_search(query, k?)` — explicit recall when the auto-\n"
    "    injected memory block at prompt-time didn't surface what you\n"
    "    need (e.g. user asks \"what did I tell you about X last week?\",\n"
