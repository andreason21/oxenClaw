...
def test_decide_skip_when_under_threshold() -> None:
    ...
    plan = decide_compaction(msgs, model_context_tokens=100_000, threshold_ratio=0.85)
...
def test_decide_picks_safe_boundary_under_pressure() -> None:
    ...
    assert all(b == a + 1 for a, b in zip(plan.drop_indexes, plan.drop_indexes[1:], strict=False))
...
def test_decide_avoids_orphaning_tool_result() -> None:
    ...
    plan = decide_compaction(msgs, model_context_tokens=100, keep_tail_turns=1, force=True)
...
async def test_apply_compaction_replaces_prefix_with_summary() -> None:
    ...
    plan = decide_compaction(msgs, model_context_tokens=1_500, keep_tail_turns=4, force=True)
    assert plan.needed
...
async def test_maybe_compact_skips_when_under_threshold() -> None:
    ...
    did = await maybe_compact(s, model_context_tokens=100_000, summarizer=truncating_summarizer)
    assert did is False
...
async def test_truncating_summarizer_includes_first_user_and_last_assistant() -> None:
    msgs = [
        ...
        AssistantMessage(content=[TextContent(text="middle answer")], stop_reason="end_turn"),
