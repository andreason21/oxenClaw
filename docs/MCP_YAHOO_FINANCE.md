...
A 5-minute walkthrough that connects sampyClaw to the third-party
...
sampyClaw skill. Configure once, the tools appear in every agent.
...
mingle with sampyClaw's runtime:
...
Add an entry to `~/.sampyclaw/mcp.json` using the standard
...
names never clash with sampyClaw's built-in `web_fetch`, `weather`, etc.
...
restart `sampyclaw gateway start` after editing `mcp.json` and the
...
- **`yfmcp.get_ticker_info` returns 30+ KB of JSON per call.** Small
  ...
  return wide payloads, or write a thin sampyClaw wrapper tool that
  returns only the 3–4 fields the model needs.
- **Stdio servers spawn one subprocess per gateway boot.** `yfmcp`
  startup is ~1 s; sampyClaw connects lazily on first use. Heavy MCP
...
- **`yfinance` itself rate-limits.** Repeat calls during a soak test
  ...
  sampyClaw's MCP client surfaces the error string and the next call
...
- **More servers**: PyPI / npm have MCP servers for GitHub, Linear,
  ...
  `mcp.json` file accepts as many entries as you want; sampyClaw
...
- **Wrap with `gated_tool`**: if an MCP tool can mutate state (file
  ...
  before execution.
- **Server-side**: exposing sampyClaw's own tools to other MCP clients
