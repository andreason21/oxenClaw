...
- `SAMPYCLAW_AUDIT_OUTBOUND=1`           — enable
- `SAMPYCLAW_AUDIT_OUTBOUND_BODY=1`      — also persist response body
- `SAMPYCLAW_AUDIT_OUTBOUND_PATH=path`   — sqlite file (default ~/.sampyclaw/outbound-audit.db)
- `SAMPYCLAW_AUDIT_OUTBOUND_SAMPLE=0.5`  — sample rate (default 1.0)
...
def should_audit_from_env(
    ...
    raw_path = src.get("SAMPYCLAW_AUDIT_OUTBOUND_PATH")
    if raw_path:
