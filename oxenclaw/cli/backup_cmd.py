"""`oxenclaw backup` subcommands — create, restore, verify, list."""
...
app = typer.Typer(
    help="Backup and restore the oxenClaw home directory.",
...
def create(
    output: Path | None = typer.Argument(
        None,
        help="Output file or directory. Default: cwd/oxenclaw-backup-<ts>.tar.gz.",
    ),
) -> None:
    """Create a snapshot of `~/.oxenclaw/`."""
...
def restore(
    ...
    """Restore a backup archive into `~/.oxenclaw/`."""
    try:
...
@app.command("home")
def home() -> None:
    """Print the oxenClaw home directory that backups capture."""
