...
oxenclaw gateway start --provider llamacpp-direct \
  --auth-token "$TOKEN"
```

> The documented default is `llamacpp-direct` serving the Unsloth
> `gemma-4-E4B-it-UD-Q4_K_XL.gguf` from Hugging Face. The empirical
> 25/25 gate below was historically measured against
> `gemma4:latest` over Ollama — that path still works, and on the
> Ollama side the recommended model is `gemma4-fc` (a custom
> Modelfile that adds a tool-calling chat template; plain
> `gemma4:latest` never emits `tool_call` blocks). Setup details:
> [`LLAMACPP_DIRECT.md`](./LLAMACPP_DIRECT.md) for the GGUF path,
> [`OLLAMA.md` → gemma3 / gemma4 function calling](./OLLAMA.md#gemma3--gemma4-function-calling--full-setup)
> for the Ollama Modelfile.
