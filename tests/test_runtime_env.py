...
def test_is_wsl_detects_wsl_marker():
    with patch("sys.platform", "linux"), patch("platform.release", return_value="5.15.0-WSL2"):
        assert is_wsl() is True
...
def test_is_wsl_false_on_native_linux():
    with (
        patch("sys.platform", "linux"),
        patch("platform.release", return_value="6.5.0-generic"),
...
def test_is_wsl_false_on_macos():
    with patch("sys.platform", "darwin"), patch("platform.release", return_value="22.6.0"):
        assert is_wsl() is False
...
def test_describe_platform_appends_wsl_marker():
    with (
        ...
        patch("platform.release", return_value="5.15.0-microsoft-standard-WSL2"),
    ):
