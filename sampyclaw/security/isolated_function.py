...
2. Spawn a fresh `python3 -m sampyclaw.security.tool_runner module:attr`
...
def _curated_pythonpath(handler_module: str) -> str:
    ...
    - the directory containing the `sampyclaw` package (so the tool_runner
      module can import).
    ...
    for name in (top_level, "sampyclaw"):
        root = _root_for(name)
...
class IsolatedFunctionTool:
    ...
    def __init__(
        ...
        *,
        ...
        # sampyclaw — never propagate the host's full site-packages.
    ...
    async def execute(self, args: dict[str, Any]) -> str:
        ...
        argv = [
            sys.executable,
            "-m",
            "sampyclaw.security.tool_runner",
            self._handler_path,
        ]
