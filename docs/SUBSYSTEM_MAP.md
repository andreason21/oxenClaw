# openclaw ↔ sampyClaw subsystem map
...
- **Status** — In: has a Python counterpart under `sampyclaw/`. Partial: started but incomplete. Out: intentionally not ported. TBD: undecided.
- **Priority** — P0 required for Phase B exit. P1 needed for Phase A. P2 defer. Out = never.

## `openclaw/src/*` → `sampyclaw/*`

| openclaw dir | ts files | sampyclaw | Status | Priority | Notes |
|---|--:|---|---|---|---|
...
| `memory-host-sdk/` | 57 | — | **Out** | P1 | retriever + embeddings + store partially covered by `sampyclaw/memory/`; revisit once A.1 lands |
...
## `openclaw/extensions/*` → `sampyclaw/extensions/*`

openclaw ships **~90 extensions**. sampyClaw ports **1** (telegram) as pilot. Full list is parked for post-Phase-B scoping.

| Category | openclaw extensions | sampyClaw |
|---|---|---|
...
| LLM providers | anthropic, anthropic-vertex, openai, google, amazon-bedrock, amazon-bedrock-mantle, alibaba, arcee, byteplus, chutes, cloudflare-ai-gateway, codex, copilot-proxy, deepgram, deepseek, elevenlabs, exa, fal, fireworks, github-copilot, groq, huggingface, kilocode, kimi-coding, lmstudio, mistral, moonshot, nvidia, ollama, openrouter, perplexity, qianfan, qwen, sglang, stepfun, synthetic, tencent, together, tokenjuice, vercel-ai-gateway, venice, vllm, volcengine, voyage, xai, zai, vydra, minimax, microsoft, microsoft-foundry, brave, searxng, tavily, firecrawl, duckduckgo, litellm, runway, comfy | anthropic provider is inline in `agents/anthropic_agent.py` (not a plugin yet) |
| Memory | active-memory, memory-core, memory-lancedb, memory-wiki | rolled into `sampyclaw/memory/` (monolithic, not pluggable yet) |
...
2. **Extensions are plug-ins, not branches of the core.** Extension parity is a per-extension decision, not a core-port task. Skip the ~85 provider extensions for sampyClaw v1 unless user specifically wants one.
